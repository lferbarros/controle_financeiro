...
# --- FUNÇÃO DE CARGA DE DADOS (COM TIPAGEM FORÇADA) ---
def carregar_dados_da_nuvem():
    try:
        df_l = pd.read_csv(csv_lanc)
        # Forçar tipos básicos
        ...
        # Garantir que a coluna Meio_Pgto exista e seja string
        if 'Meio_Pgto' not in df_l.columns:
            df_l['Meio_Pgto'] = ""
        df_l['Meio_Pgto'] = df_l['Meio_Pgto'].fillna("").astype(str)
...
if 'dados_carregados' not in st.session_state:
    ...
    if res:
        ...
        # Tabela reserva com tipos explícitos para evitar o erro de compatibilidade
        st.session_state.df_lanc = pd.DataFrame({
            'Data': [datetime.now().date()],
            'Categoria': [""],
            'Valor': [0.0],
            'Meio_Pgto': [""]
        }).astype({'Categoria': str, 'Meio_Pgto': str, 'Valor': float})
...
def gerar_extrato(lanc, cats, saldo_ini):
    if lanc.empty:
        return pd.DataFrame(columns=['Data', 'Categoria', 'Meio_Pgto', 'Valor', 'Saldo_Acumulado'])
    
    df = lanc[['Data', 'Categoria', 'Valor', 'Meio_Pgto']].copy()
    
    # GARANTIA DE TIPO: Converte Meio_Pgto para string explicitamente
...
lista_cats = st.session_state.df_cats['Categoria'].dropna().astype(str).unique().tolist()
lista_meios = st.session_state.df_meios['Meio_Pagamento'].dropna().astype(str).unique().tolist()

df_viz = gerar_extrato(st.session_state.df_lanc, st.session_state.df_cats, st.session_state.saldo_ini)

# --- TABELA PRINCIPAL (ONDE DAVA O ERRO) ---
